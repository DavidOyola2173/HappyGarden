...
def draw():
  global game_over, time_elapse, finalised
  if not game_over:
    screen.clear()
    ...
    for fangflower in fangflower_list:
      fangflower.draw()
    time_elapse = int(time.time() - start_time)
    screen.draw.text(
      "Jardin Feliz por: " + str(time_elapse) + " segundos",
      topleft=(10,10), color="black"
    )
  else:
    if not finalised:
      cow.draw()
      screen.draw.text("Jardin Feliz por: "+str(time_elapse)+ " segundos", topleft=(10,10), color="black")
      if (not garden_happy):
        screen.draw.text("Jardin Triste - GAME OVER!", color = "black", topleft=(10,50))
        finalised = True
      else:
        screen.draw.text("Ataque de la flor malvada - GAME OVER", color="black", topleft=(10,50))
        finalised = True
  return
...
def update():
  ...
  global flower_list, fangflower_list, time_elapse
  ...
  if not game_over:
    ...
    if time_elapse > 15 and not fangflower_list:
      mutate()
    update_fangflowers()
